...
The  permissions we are interested in granting are listed below. Review the following table for a detailed description of the differences between `All` or `OwnedBy`. Depending on the granularity you want select to choose `All` or `OwnedBy`. Both permissions can be used for a *Super Service Principal*.
